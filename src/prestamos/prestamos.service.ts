...
export class PrestamosService {
  ...
  async create(dto: CreatePrestamoDto) {
    ...
    const prestamo = await this.prisma.prestamo.create({
      ...
      include: { usuario: { select: { id: true, nombre: true, email: true } }, libro: true },
    });
  ...
  async devolver(id: number) {
    ...
    if (ahora > prestamo.fechaDevolucion) {
      const diasRetraso = Math.ceil((ahora.getTime() - prestamo.fechaDevolucion.getTime()) / (1000 * 60 * 60 * 24));
      const monto = diasRetraso * 0.5; // $0.50 por día
